{% macro get_unit_test_sql(model, input_mapping, depends_on) %}
    ...
    {% if execute %}
        ...
        {% set ns.graph_model = dbt_datamocktool.__get_graph_model(
            project_name, model.schema, model.name
        ) %}
        ...
        {% set mock_model_relation = dbt_datamocktool._get_model_to_mock(
            model,
            suffix=(
                "_dmt_" ~ modules.datetime.datetime.now().strftime("%S%f")
            ),
        ) %}

        {% do dbt_datamocktool._create_mock_table_or_view(
            mock_model_relation, ns.test_sql
        ) %}
...
{% macro get_unit_test_incremental_sql(model, input_mapping, depends_on) %}
    ...
    {% if execute %}
        ...
        {% do dbt_datamocktool._create_mock_table_or_view(
            mock_model_relation, "select * from " ~ input_mapping.this
        ) %}
        ...
        {% do dbt_datamocktool._create_mock_merge_table(
            mock_model_relation,
            "(" + ns.test_sql + ")",
            dest_columns=adapter.get_columns_in_relation(mock_model_relation),
...
{% macro _get_model_to_mock(model, suffix) %}
    {{
        return(
            adapter.dispatch("_get_model_to_mock", "dbt_datamocktool")(model, suffix)
        )
    }}
{% endmacro %}
...
{% macro spark___get_model_to_mock(model, suffix) %}
    {{
        return(
            make_temp_relation(
                model.incorporate(type="table").include(schema=False), suffix=suffix
            )
        )
    }}
{% endmacro %}
...
{% macro sqlserver___get_model_to_mock(model, suffix) %}
    ...
    {% if not adapter.check_schema_exists(database=model.database, schema=schema) %}
        {% do adapter.create_schema(
            api.Relation.create(database=model.database, schema=schema)
        ) %}
    {% endif %}
    ...
    {{
        return(
            model.incorporate(
                type="view", path={"identifier": tmp_identifier, "schema": schema}
            )
        )
    }}
{% endmacro %}
...
{% macro _create_mock_table_or_view(model, test_sql) %}
    {{
        return(
            adapter.dispatch("_create_mock_table_or_view", "dbt_datamocktool")(
                model, test_sql
            )
        )
    }}
...
{% macro _create_mock_merge_table(model, test_sql, dest_columns, unique_key) %}
    {{
        return(
            adapter.dispatch("_create_mock_merge_table", "dbt_datamocktool")(
                model, test_sql, dest_columns, unique_key
            )
        )
    }}
{% endmacro %}
...
{% macro default___create_mock_merge_table(model, test_sql, dest_columns, unique_key) %}
    {% do run_query(
        get_merge_sql(
            model, test_sql, dest_columns=dest_columns, unique_key=unique_key
        )
    ) %}
{% endmacro %}
    ...
    {% if graph_model is none %}
        {% for node in graph.nodes.values() %}
            {% if node.alias == model_name and node.schema == model_schema %}
                {% set graph_model = node %} {{ return(graph_model) }}
            {% endif %}
        {% endfor %}
    {% endif %}
    {{ return(graph_model) }}
