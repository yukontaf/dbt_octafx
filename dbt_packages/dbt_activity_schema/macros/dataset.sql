{% macro dataset(activity_stream, primary_activity, appended_activities=[]) %}
    {{
        return(
            adapter.dispatch("dataset", "dbt_activity_schema")(
                activity_stream, primary_activity, appended_activities
            )
        )
    }}
{% endmacro %}
...
#}
    ...
    with
        ...
        filter_activity_stream_using_primary_activity as (
            ...
            where
                {{ primary() }}.{{ columns.activity }}
                = {{ dbt.string_literal(primary_activity.name) }}
            ...
            {{ alias_cte(activity, i) }} as (
                select
                    ...
                    {% for col in activity.included_columns %}
                        {%- set parsed_col = dbt_activity_schema.parse_column(
                            appended(), col
                        ) -%}
                        {% call activity.relationship.aggregation_func() %}
            {{ parsed_col }}
                        {% endcall %} as
                        {{ dbt_activity_schema.alias_appended_activity(activity, col) }}
                        {% if not loop.last %},{% endif %}
                    {% endfor %}
                ...
                left join
                    {{ activity_stream }} as {{ appended() }}
                    on (
                        -- Join on Customer UUID Column
                        {{ appended() }}.{{ columns.customer }}
                        = {{ primary() }}.{{ columns.customer }}

                        -- Join the Correct Activity
                        and {{ appended() }}.{{- columns.activity }}
                        = {{ dbt.string_literal(activity.name) }}
                            ...
                            {% if activity.relationship.name == "nth_ever" %}
                                {{
                                    activity.relationship.join_clause(
                                        activity.relationship.nth_occurance
                                    )
                                }}
                            {% else %}{{ activity.relationship.join_clause() }}
                            {% endif %}
                        )
        ...
        rejoin_aggregated_activities as (
            select
                ...
                {% for activity in appended_activities %}
                    {% set i = loop.index %} {% set last_outer_loop = loop.last %}
                    {% for col in activity.included_columns %}
                        {{ alias_cte(activity, i) }}.
                        {{ alias_appended_activity(activity, col) }}
            ...
            {% for activity in appended_activities %}
                ...
                left join
                    {{ alias_cte(activity, i) }}
                    on {{ alias_cte(activity, i) }}.{{ columns.activity_id }}
                    = {{ primary() }}.{{ columns.activity_id }}

            {% endfor %}
        )

    select *
    from rejoin_aggregated_activities
