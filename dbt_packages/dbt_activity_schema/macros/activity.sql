{% macro activity(
    relationship,
    activity_name,
    included_columns=var(
        "included_columns",
        var("dbt_activity_schema", {}).get(
            "included_columns",
            dbt_activity_schema.columns().values() | list
        ),
    ),
    ...
    {{
        return(
            adapter.dispatch("activity", "dbt_activity_schema")(
                relationship,
                activity_name,
                included_columns,
...
{% macro default__activity(
    relationship, activity_name, included_columns, additional_join_condition
) %}
