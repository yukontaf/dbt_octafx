{% macro connect_ordered_graph(
    ...
    with
        ...
        enforce_graph_types as (
            select
                cast(
                    {{ graph_id if graph_id else "1" }} as {{ type_string() }}
                ) as graph_id,
                ...
                {% else %} cast({{ ordering_field }} as {{ ordering_type }}) as ordering
                {% endif %}
            from {{ input }}
        ),
        ...
        from_vertices as (
            ...
            inner join
                subgraphs
                on _input.graph_id = subgraphs.graph_id
                and _input.vertex_1 = subgraphs.vertex
        ),
        ...
        to_vertices as (
            ...
            inner join
                subgraphs
                on _input.graph_id = subgraphs.graph_id
                and _input.vertex_2 = subgraphs.vertex
        ),

        vertex_ordering as (
            select graph_id, vertex, ordering
            from from_vertices {{ dbt_graph_theory.set_union(distinct=true) }}
            select graph_id, vertex, ordering
            from to_vertices
        ),
        ...
        subgraph_max_min_ordering as (
            ...
            from subgraphs
            inner join
                vertex_min_max_ordering as orderings
                on subgraphs.graph_id = orderings.graph_id
                and subgraphs.vertex = orderings.vertex
            group by
                subgraphs.graph_id, subgraphs.subgraph_id, subgraphs.subgraph_members
        ),
        ...
        subgraph_lead_lags as (
            select
                ...
                min_ordering,
                max_ordering,
                lag(subgraph_id) over (
                    partition by graph_id order by min_ordering
                ) as lag_subgraph_id,
                lag(max_ordering) over (
                    partition by graph_id order by min_ordering
                ) as lag_max_ordering
            from subgraph_max_min_ordering
        ),
        ...
        new_edges_join as (
            select
                ...
                -- the join condition ensure that we need to be able to dedupe a >
                -- condition, we use this field to do that - ie. we pick the most
                -- recent
                -- "to_vertices" row that has joined (ie. is less than the required
                -- max ordering (min_ordering in the next subgraph))
                row_number() over (
                    partition by to_vertices.graph_id, to_vertices.subgraph_id
                    order by to_vertices.ordering desc
                )
                = 1 as is_max_to_vertex_joined
            from subgraph_lead_lags as _required_edges
            inner join
                from_vertices
                -- this is the vertex in the subgraph that has the min ordering
                on _required_edges.graph_id = from_vertices.graph_id
                and _required_edges.subgraph_id = from_vertices.subgraph_id
                and _required_edges.min_ordering = from_vertices.ordering
            inner join
                to_vertices
                -- these are the vertices in the previous subgraph that have an
                -- ordering less than the min of the subgraph
                on _required_edges.graph_id = to_vertices.graph_id
                and _required_edges.lag_subgraph_id = to_vertices.subgraph_id
                and _required_edges.min_ordering > to_vertices.ordering
            -- filter out the "first" subgraph id - the remainder is exactly the
            -- number of new edges required.
        ...
        include_new_edges as (
            ...
            from enforce_graph_types {{ dbt_graph_theory.set_union(distinct=false) }}
            select
                {{ 'graph_id as ' ~ graph_id ~ ',' if graph_id }}
                concat(
                ...
                {% if ordering_type == "timestamp" %}
                    case
                        when
                            ...
                            or old_ordering
                            = {{ dbt_utils.dateadd("second", "-1", "new_ordering") }}
                        then new_ordering
                ...
                {% elif ordering_type == "date" %}
                    case
                        when
                            new_ordering = old_ordering
                            or old_ordering
                            = {{ dbt_utils.dateadd("day", "-1", "new_ordering") }}
                        then new_ordering
