{% macro generate_subgraph_id() %}
    {{
        return(
            adapter.dispatch(
                "generate_subgraph_id", macro_namespace="dbt_graph_theory"
            )()
        )
    }}
{% endmacro %}
...
{% macro default__generate_subgraph_id() %}
    select
        ...
        cast(
            dense_rank() over (
                partition by graph_id order by subgraph_members
            ) as {{ type_string() }}
        ) as subgraph_id
    from node_subgraphs
{% endmacro %}
...
{% macro largest_connected_subgraphs(
    ...
    with recursive
        ...
        all_vertices as (
            select
                {{ graph_id if graph_id else "cast('1' as " ~ type_string() ~ ")" }}
                as graph_id,
                {{ vertex_1 }} as vertex
            from enforce_graph
            where
                {{ vertex_1 }} is not null
        ...
        all_edges as (
            ...
            from enforce_graph
            where
                coalesce({{ vertex_1 }} != {{ vertex_2 }}, true)
                and ({{ vertex_1 }} is not null or {{ vertex_2 }} is not null)
                {{ dbt_graph_theory.set_union(distinct=true) }}
            select
                {{ graph_id if graph_id else "cast('1' as " ~ type_string() ~ ")" }}
                as graph_id,
        ...
        graph_walk as (
            select
                ...
                all_edges.vertex_2,
                {{
                    dbt_graph_theory.array_construct(
                        components=["all_edges.vertex_1", "all_edges.vertex_2"]
                    )
                }} as path_array
            from all_edges
            inner join
                all_vertices
                on all_vertices.graph_id = all_edges.graph_id
                and all_vertices.vertex = all_edges.vertex_1
            ...
            select
                ...
                all_edges.vertex_2,
                {{
                    dbt_graph_theory.array_append(
                        array="graph_walk.path_array", new_value="all_edges.vertex_2"
                    )
                }} as path_array
            from all_edges
            inner join
                graph_walk
                -- walk from the "end" vertex of the last edge to the "start" vertex
                -- of the next edge
                -- only walk there if the target vertex has not already been reached
                -- on the walk
                -- note: while this does not guarantee full coverage on each path, it
                -- means that every reachable vertex from every original vertex has a
                -- row.
                on graph_walk.graph_id = all_edges.graph_id
                and graph_walk.vertex_2 = all_edges.vertex_1
                and not (
                    {{
                        dbt_graph_theory.array_contains(
                            array="graph_walk.path_array", value="all_edges.vertex_2"
                        )
                    }}
                )
        ),
        ...
        all_paths as (
            select graph_id, orig_vertex, vertex_1 as end_vertex
            from graph_walk {{ dbt_graph_theory.set_union(distinct=true) }}
            select graph_id, orig_vertex, vertex_2 as end_vertex
            from graph_walk
        ),
        ...
        node_subgraphs as (
            ...
            from all_paths
            group by graph_id, orig_vertex
        ),

        generate_subgraph_id as ({{ dbt_graph_theory.generate_subgraph_id() }}),
        ...
        largest_connected_subgraphs as (
            ...
            from node_subgraphs
            left join
                generate_subgraph_id
                on node_subgraphs.graph_id = generate_subgraph_id.graph_id
                and node_subgraphs.vertex = generate_subgraph_id.vertex
        )

    select *
    from largest_connected_subgraphs
{% endmacro %}
