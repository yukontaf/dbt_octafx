...
#}
    {{
        return(
            adapter.dispatch("sql_values", macro_namespace="dbt_graph_theory")(
                data, metadata, table_alias
            )
        )
    }}
{% endmacro %}
                            ...
                            (
                                {% for col in row -%}
                                    {% if has_names and outer_loop.first %}
                                        cast(
                                            {{ col }}
                                            as {{ metadata["types"][loop.index0] }}
                                        )
                                        as {{ metadata["names"][loop.index0] }} {{ ',' if not loop.last }}
                                    {% else %}
                                        cast(
                                            {{ col }}
                                            as {{ metadata["types"][loop.index0] }}
            ...
            {% for row in data -%}
                {% if has_types %}
                    ...
                    (
                        {% for col in row -%}
                            cast(
                                {{ col }} as {{ metadata["types"][loop.index0] }}
                            ) {{ ',' if not loop.last }}
                        {%- endfor %}
                    )
                {% else %} ({{ row | join(", ") }}) {{ ',' if not loop.last }}
                {% endif %}
            {% endfor %}
    ) as {{ table_alias }}
