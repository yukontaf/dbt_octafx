{% macro merge_dbt_configs(dbt_config_1, dbt_config_2) %}
    {% if dbt_config_1 == None %} {{ return(dbt_config_2) }} {% endif %}

    {% if dbt_config_2 == None %} {{ return(dbt_config_1) }} {% endif %}
    ...
    {% for resource_type in dbt_config_1.keys() | list + dbt_config_2.keys() | list %}
        ...
        {% if resource_type not in resource_types and resource_type in [
            "models",
            "sources",
            "seeds",
        ] %}
            ...
            {% for model_name in model_names %}
                ...
                {% if model_name in config_1_model_lookup.keys() %}
                    {% if "tests" in config_1_model_lookup[model_name].keys() %}
                        {% for model_test in config_1_model_lookup[model_name][
                            "tests"
                        ] %}
                ...
                {% if model_name in config_2_model_lookup.keys() %}
                    {% if "tests" in config_2_model_lookup[model_name].keys() %}
                        {% for model_test in config_2_model_lookup[model_name][
                            "tests"
                        ] %}
                            {% do model_tests.append(model_test) %}
                        {% endfor %}
                ...
                {% for col_name in col_names %}
                    ...
                    {% else %}
                        {% for k, v in config_1_col_lookup[
                            col_name
                        ].items() | list + config_2_col_lookup[
                            col_name
                        ].items() | list %}
                            {% do new_column.update({k: v}) %}
                        {% endfor %}
                        {% set col_tests = (
                            config_1_col_lookup[col_name]["tests"]
                            + config_2_col_lookup[col_name]["tests"]
                        ) %}
                    {% endif %}
