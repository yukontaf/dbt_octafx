{% macro get_uniqueness_test_suggestions(
    ...
    {% if execute %}
        {{
            return(
                adapter.dispatch("get_uniqueness_test_suggestions", "testgen")(
                    table_relation,
                    sample,
                    limit,
                    resource_type,
...
{% macro default__get_uniqueness_test_suggestions(
    ...
    {% set column_names = [] %}
    {% for col in columns %} {% do column_names.append(col.column) %} {% endfor %}

    {% if column_names | length == 0 %} {{ return(dbt_config) }} {% endif %}
    ...
    {% if limit %} {% set limit_expr = "LIMIT " ~ limit | string %}
    {% else %} {% set limit_expr = "" %}
    {% endif %}
    ...
    {% if limit != None %}
        {% if sample == true %}
            {% set limit_stmt = (
                "ORDER BY "
                ~ testgen.get_random_function()
                ~ "() LIMIT "
                ~ limit
            ) %}
        {% else %} {% set limit_stmt = "LIMIT " ~ limit %}
        {% endif %}
    {% else %} {% set limit_stmt = "" %}
    {% endif %}
    ...
    {% for column_combo in column_combinations %}
        ...
        {% do count_distinct_exprs.append(
            "SELECT "
            ~ loop.index
            ~ " AS ORDERING, count(1) AS CARDINALITY             from (                 SELECT 1 FROM base                 GROUP BY "
            ~ column_combo_quoted
            | join(", ") ~ "             ) t"
        ) %}
    {% endfor %}
    ...
    {% set cardinality_results = zip(
        column_combinations, testgen.query_as_list(count_distinct_sql)
    ) %}
    ...
    {% for unique_key in unique_keys %}
        ...
        {% else %}
            ...
            {% for key_length in range(unique_key | length) %}
                {% if key_length > 0: %}
                    {% for perm in modules.itertools.permutations(
                        unique_key, key_length
                    ) %}
    ...
    {% for unique_key in deduped_unique_keys %}
        {% if unique_key | length == 1 %}
            ...
            {% set col_config = {"name": unique_key[0], "tests": tests} %}
    ...
    {% if table_tests != [] %} {% do model.update({"tests": table_tests}) %} {% endif %}
