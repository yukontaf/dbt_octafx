{% macro get_range_test_suggestions(
    ...
    {% if execute %}
        {{
            return(
                adapter.dispatch("get_range_test_suggestions", "testgen")(
                    table_relation,
                    sample,
                    limit,
                    resource_type,
...
{% macro default__get_range_test_suggestions(
    ...
    {% for column in columns %}
        {% if column.is_number() %} {% do number_cols.append(column) %} {% endif %}
    {% endfor %}

    {% if number_cols | length == 0 %} {{ return(dbt_config) }} {% endif %}
            ...
            "tests": [
                {
                    "dbt_utils.accepted_range": {
                        "min_value": testgen.cast_number(
                            min_val - (stddevs * stddev / 2)
                        ),
                        "max_value": testgen.cast_number(
                            max_val + (stddevs * stddev / 2)
                        ),
                    }
                }
            ],
