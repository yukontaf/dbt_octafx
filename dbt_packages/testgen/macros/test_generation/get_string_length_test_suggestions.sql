{% macro get_string_length_test_suggestions(
    ...
    {% if execute %}
        {{
            return(
                adapter.dispatch("get_string_length_test_suggestions", "testgen")(
                    table_relation,
                    sample,
                    limit,
                    resource_type,
...
{% macro default__get_string_length_test_suggestions(
    ...
    {# {{ print(string_cols) }} #}
    {% if string_cols | length == 0 %} {{ return(dbt_config) }} {% endif %}
    ...
    {% for result in results %}
        ...
        {% if min_val == max_val %}
            {% set test = {
                "dbt_expectations.expect_column_value_lengths_to_equal": {
                    "value": min_val,
                    "row_condition": adapter.quote(result[0])
                    ~ " is not null",
                }
            } %}
        {% else %}
        ...
        {% set col_config = {"name": result[0], "tests": [test]} %}
