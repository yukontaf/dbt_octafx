...
{% macro default__get_accepted_values_test_suggestions(
    ...
    {# {{ print(columns) }} #}
    {% if columns | length == 0 %} {{ return(dbt_config) }} {% endif %}
    ...
    {% for column in columns %}
        ...
        {% do count_distinct_exprs.append(
            ...
            ~ " AS ORDERING,                  '"
            ~ column.column
            ~ "' AS COLNAME,                  count(1) as CARDINALITY, "
            ~ testgen.sql_agg_array(column.column)
            ~ " AS UNIQUE_VALUES             from (                 select "
    ...
    {% for cardinality_result in cardinality_results %}
        ...
        {% set col_config = {
            ...
            "tests": [
                {
                    "accepted_values": {
                        "values": fromjson(cardinality_result[3]) | sort
                    }
                }
            ],
        } %}
    ...
    {% set new_dbt_config = {
        resource_type: [
            {"name": table_relation.identifier, "columns": column_tests}
        ]
    } %}
