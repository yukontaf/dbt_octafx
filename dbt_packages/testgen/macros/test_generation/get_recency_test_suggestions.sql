...
{% macro default__get_recency_test_suggestions(
    ...
    {% if timestamp_cols | length == 0 %} {{ return(dbt_config) }} {% endif %}
    ...
    {% for result in recency_results %}
        ...
        {% if interval >= 60 * 24 %}
            {% set datepart = "day" %} {% set interval = interval / (60 * 24) %}
        {% elif interval >= 60 %}
            {% set datepart = "hour" %} {% set interval = interval / 60 %}
        {% else %} {% set datepart = "minute" %}
        {% endif %}
