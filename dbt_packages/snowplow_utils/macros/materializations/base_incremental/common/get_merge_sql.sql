...
{% macro default__get_merge_sql(
    ...
    {% if optimize %}
        ...
        -- run some queries to dynamically determine the min + max of this
        -- 'upsert_date_key' in the new data
        ...
        -- use those calculated min + max values to limit 'target' scan, to only the
        -- days with new data
        {% set predicate_override %}
    ...
    {% set predicates = (
        [predicate_override] + orig_predicates
        if predicate_override
        else orig_predicates
    ) %}
    -- standard merge from here
    {% if target.type in ["databricks", "spark"] -%}
        {% set merge_sql = spark__get_merge_sql(
            target_tb, source, unique_key, dest_columns, predicates
        ) %}
    {% else %}
        {% set merge_sql = dbt.default__get_merge_sql(
            target_tb, source, unique_key, dest_columns, predicates
        ) %}
    {% endif %}
...
{% macro default__get_delete_insert_merge_sql(
    target_tb, source, unique_key, dest_columns, incremental_predicates
) -%}
    {# Set default predicates to pass on #}
    {%- set predicate_override = "" -%}
    {%- set orig_predicates = (
    ...
    -- standard merge from here
    {% if target.type in ["databricks", "spark"] -%}
        {% set merge_sql = spark__get_delete_insert_merge_sql(
            target_tb, source, unique_key, dest_columns, predicates
        ) %}
    {% else %}
        {% set merge_sql = dbt.default__get_delete_insert_merge_sql(
            target_tb, source, unique_key, dest_columns, predicates
        ) %}
    {% endif %}
