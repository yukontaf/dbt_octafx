...
{% macro combine_column_versions(
    ...
    {% set level_limit = snowplow_utils.get_level_limit(
        nested_level, level_filter, required_field_names
    ) %}

    {# Limit level_limit to max_nested_level if required #}
    {% set level_limit = (
        max_nested_level
        if level_limit is none or level_limit > max_nested_level
        else level_limit
    ) %}

    {%- set matched_columns = (
        snowplow_utils.get_columns_in_relation_by_column_prefix(
            relation, column_prefix
        )
    ) -%}

    {# Removes excluded versions, technically removes any column with that suffix #}
    {%- set filter_columns_by_version = snowplow_utils.exclude_column_versions(
        matched_columns, exclude_versions
    ) -%}
    ...
    {% for column in filter_columns_by_version | sort(attribute="name", reverse=true) %}
        {% set flattened_fields = snowplow_utils.flatten_fields(
    ...
    {% set merged_fields = snowplow_utils.merge_fields_across_col_versions(
        flattened_fields_by_col_version
    ) %}

    {# Filters merged_fields based on required_fields if provided, or the level filter if provided. Default return all fields. #}
    {% set matched_fields = snowplow_utils.get_matched_fields(
    ...
    {% for field in matched_fields %}
        ...
        {% set passed_field_alias = (
            required_fields
            | selectattr(0, "equalto", field.field_name)
            | map(attribute=1)
            | list
        ) %}
        ...
        {% set field_alias = (
            default_field_alias
            if not passed_field_alias | length
            else passed_field_alias[0]
        ) %}

        {# Coalesce each field's path across all version of columns, ordered by latest col version. #}
        {% set coalesced_field_path = snowplow_utils.coalesce_field_paths(
            paths=field.field_paths,
