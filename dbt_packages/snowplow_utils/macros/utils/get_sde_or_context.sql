...
{% macro get_sde_or_context(
    ...
    {{
        return(
            adapter.dispatch("get_sde_or_context", "snowplow_utils")(
                schema,
                identifier,
                lower_limit,
                upper_limit,
...
{% macro default__get_sde_or_context(
    ...
    {% if execute %}
        {% do exceptions.raise_compiler_error(
            "Macro get_sde_or_context is only for Postgres or Redshift, it is not supported for"
            ~ target.type
        ) %}
    {% endif %}
{% endmacro %}
...
{% macro postgres__get_sde_or_context(
    schema,
    identifier,
    lower_limit,
    upper_limit,
