...
{% macro default__parse_agg_dict(agg_dict) %}
    ...
    {% if agg_type not in [
        ...
        {{
            exceptions.raise_compiler_error(
                "Snowplow Error: Unexpected aggregation provided, must be one of sum, avg, min, max, count, countd (count distinct), not "
                ~ agg_type
                ~ "."
            )
        }}
    ...
    {% endif %}
    {% if agg_type == "countd" %} count(distinct {{ agg_field }}) as {{ agg_alias }}
    {% else %} {{ agg_type }} ({{ agg_field }}) as {{ agg_alias }}
    {% endif %}
