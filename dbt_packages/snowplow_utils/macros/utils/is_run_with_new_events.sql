...
#}
{% macro is_run_with_new_events(
    package_name,
    new_event_limits_table=none,
    incremental_manifest_table=none,
    base_sessions_lifecycle_table=none
) %}

    {%- if new_event_limits_table -%}
        {%- set new_event_limits_relation = ref(new_event_limits_table) -%}
    {%- else -%}
        {%- set new_event_limits_relation = (
            snowplow_utils.get_new_event_limits_table_relation(package_name)
        ) -%}
    {%- endif -%}
    ...
    {%- else -%}
        {%- set incremental_manifest_relation = (
    ...
    {% if is_incremental() %}
        ...
        {%- if base_sessions_lifecycle_relation -%}
            {%- set base_sessions_lifecycle_identifier = ref(
