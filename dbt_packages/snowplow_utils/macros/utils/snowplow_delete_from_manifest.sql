...
{% macro snowplow_delete_from_manifest(models, incremental_manifest_table) %}
    ...
    {%- if models is string -%} {%- set models = [models] -%} {%- endif -%}

    {# No models to delete or not in execute mode #}
    {% if not models | length or not execute %} {{ return("") }} {% endif %}

    {# Get the manifest table to ensure it exits #}
    {%- set incremental_manifest_table_exists = adapter.get_relation(
        incremental_manifest_table.database,
    ...
    {%- if not incremental_manifest_table_exists -%}
        {{
            return(
                dbt_utils.log_info(
                    "Snowplow: " + incremental_manifest_table
    ...
    {%- set models_in_manifest = dbt_utils.get_column_values(
        table=incremental_manifest_table, column="model"
    ) -%}
    {%- set unmatched_models, matched_models = [], [] -%}

    {%- for model in models -%}

        {%- if model in models_in_manifest -%} {%- do matched_models.append(model) -%}
        {%- else -%} {%- do unmatched_models.append(model) -%}
        {%- endif -%}
    ...
    {%- if not matched_models | length -%}
        {{
            return(
                dbt_utils.log_info(
                    "Snowplow: None of the supplied models exist in the manifest"
                )
            )
        }}
    {%- endif -%}
    ...
    {%- if matched_models | length -%}
        {% do snowplow_utils.log_message(
            "Snowplow: Deleted models "
            + snowplow_utils.print_list(matched_models)
            + " from the manifest"
        ) %}
    {%- endif -%}
    ...
    {%- if unmatched_models | length -%}
        {% do snowplow_utils.log_message(
            "Snowplow: Models "
            + snowplow_utils.print_list(unmatched_models)
            + " do not exist in the manifest"
        ) %}
    {%- endif -%}
...
{% macro snowplow_web_delete_from_manifest(models) %}
    ...
    {{
        snowplow_utils.snowplow_delete_from_manifest(
            models, ref("snowplow_web_incremental_manifest")
        )
    }}
...
{% macro snowplow_mobile_delete_from_manifest(models) %}
    ...
    {{
        snowplow_utils.snowplow_delete_from_manifest(
            models, ref("snowplow_mobile_incremental_manifest")
        )
    }}

{% endmacro %}

{% macro snowplow_base_delete_from_manifest(
    models, incremental_manifest="snowplow_incremental_manifest"
) %}

    {{
        snowplow_utils.snowplow_delete_from_manifest(
            models, ref(incremental_manifest)
        )
    }}
