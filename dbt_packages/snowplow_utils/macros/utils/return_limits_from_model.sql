...
{% macro return_limits_from_model(model, lower_limit_col, upper_limit_col) -%}
    ...
    {% set target_relation = adapter.get_relation(
        database=model.database, schema=model.schema, identifier=model.name
    ) %}
    ...
    {% if target_relation is not none %}
      ...
      from {{ model }}
        ...
        {% if execute %}
            ...
            {%- if results.columns[0].values()[0] is none or results.columns[
                1
            ].values()[0] is none -%}
                ...
                {% do exceptions.warn(
                    "Snowplow Warning: No data in "
                    ~ this
                    ~ " for date range from variables, please modify your run variables to include data if this is not expected."
                ) %}
                {{ snowplow_utils.log_message("Snowplow Warning: *************") }}
                {# This allows for bigquery to still run the same way the other warehouses do, but also ensures no data is processed #}
                {% set lower_limit = snowplow_utils.cast_to_tstamp(
                    "9999-01-01 00:00:00"
                ) %}
                {% set upper_limit = snowplow_utils.cast_to_tstamp(
                    "9999-01-02 00:00:00"
                ) %}
            {%- else -%}
                {% set lower_limit = snowplow_utils.cast_to_tstamp(
    ...
    {% else %}
        ...
        {% do exceptions.warn(
            "Snowplow Warning: "
            ~ model
            ~ " does not exist. This is expected if you are compiling a fresh installation of the dbt-snowplow-* packages."
        ) %}
