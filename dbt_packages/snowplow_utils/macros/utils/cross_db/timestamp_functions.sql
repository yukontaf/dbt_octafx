...
{% macro timestamp_diff(first_tstamp, second_tstamp, datepart) %}
    {{
        return(
            adapter.dispatch("timestamp_diff", "snowplow_utils")(
...
{% macro timestamp_add(datepart, interval, tstamp) %}
    {{
        return(
            adapter.dispatch("timestamp_add", "snowplow_utils")(
                datepart, interval, tstamp
            )
        )
    }}
{% endmacro %}
...
{% macro cast_to_tstamp(tstamp_literal) -%}
    ...
    {% else %} cast('{{tstamp_literal}}' as {{ type_timestamp() }})
    {% endif %}
{%- endmacro %}
...
{%- macro bigquery__to_unixtstamp(tstamp) -%} unix_seconds({{ tstamp }}) {%- endmacro %}

{%- macro spark__to_unixtstamp(tstamp) -%} unix_timestamp({{ tstamp }}) {%- endmacro %}
...
{% macro default__current_timestamp_in_utc() %} {{ current_timestamp() }} {% endmacro %}
