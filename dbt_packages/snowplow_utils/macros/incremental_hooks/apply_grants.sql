...
{% macro default__apply_grants(relation, grant_config, should_revoke=True) %}
    ...
    #}
    {% if (
        grant_config.get("select", []) or var("snowplow__grant_select_to", [])
    ) and target.type != "bigquery" %}
        {# Add our config to the grants from our variable #}
        {% do grant_config.update(
            {
                "select": grant_config.get("select", [])
                + var("snowplow__grant_select_to", [])
            }
        ) %}
    {% endif %}
...
{% macro grant_usage_on_schemas_built_into(enabled=false) -%}
    ...
    {{
        return(
            adapter.dispatch("grant_usage_on_schemas_built_into", "snowplow_utils")(
                enabled
            )
        )
    }}
