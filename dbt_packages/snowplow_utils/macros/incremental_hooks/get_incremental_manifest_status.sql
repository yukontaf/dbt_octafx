    ...
    {% if target_relation is not none %}
      ...
      where model in ({{ snowplow_utils.print_list(models_in_run) }})
        ...
        {% if execute %}
            ...
            {% set has_matched_all_models = (
                true
                if models_matched_from_manifest == models_in_run | length
                else false
            ) %}
...
{% macro print_run_limits(run_limits_relation, package=none) -%}
    ...
    {% if execute %}
        ...
        {% set lower_limit = snowplow_utils.tstamp_to_str(
            results.columns[0].values()[0]
        ) %}
        {% set upper_limit = snowplow_utils.tstamp_to_str(
            results.columns[1].values()[0]
        ) %}
        {% set run_limits_message = (
            "Snowplow: Processing data between "
            + lower_limit
            + " and "
            + upper_limit
        ) %}
