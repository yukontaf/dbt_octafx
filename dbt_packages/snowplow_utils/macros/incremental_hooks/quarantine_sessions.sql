...
{% macro quarantine_sessions(package_name, max_session_length, src_relation=this) %}
    ...
    {{
        return(
            adapter.dispatch("quarantine_sessions", "snowplow_utils")(
                package_name, max_session_length, src_relation=this
            )
        )
    }}
...
{% macro default__quarantine_sessions(
    package_name, max_session_length, src_relation=this
) %}

    {% set quarantined_sessions = ref(package_name ~ "_base_quarantined_sessions") %}

    {% set sessions_to_quarantine_sql = snowplow_utils.get_quarantine_sql(
...
{% macro postgres__quarantine_sessions(
    package_name, max_session_length, src_relation=this
) %}
