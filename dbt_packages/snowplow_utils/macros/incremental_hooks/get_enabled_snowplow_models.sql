...
#}
{% macro get_enabled_snowplow_models(
    package_name,
    graph_object=none,
    models_to_run=var("models_to_run", ""),
    base_events_table_name="snowplow_base_events_this_run"
) -%}

    {# Override dbt graph object if graph_object is passed. Testing purposes #}
    {% if graph_object is not none %} {% set graph = graph_object %} {% endif %}

    {# models_to_run optionally passed using dbt ls command. This returns a string of models to be run. Split into list #}
    {% if models_to_run | length %} {% set selected_models = models_to_run.split(" ") %}
    {% else %} {% set selected_models = none %}
    {% endif %}
    ...
    {% set snowplow_events_this_run_path = (
        "model." + project_name + "." + base_events_table_name
    ) %}

    {% if execute %}

        {% set nodes = graph.nodes.values() | selectattr(
    ...
    #}
            {%- do exceptions.raise_compiler_error(
                "Snowplow Warning: Untagged models referencing '"
                + snowplow_events_this_run_path
                + "'. Please refer to the Snowplow docs on tagging. "
                + "Models: "
                + ", ".join(untagged_snowplow_models)
            ) -%}
