...
#}
{% macro base_create_snowplow_events_this_run(
    sessions_this_run_table="snowplow_base_sessions_this_run",
    session_identifiers=[{"schema": "atomic", "field": "domain_sessionid"}],
    session_sql=none,
    session_timestamp="load_tstamp",
    derived_tstamp_partitioned=true,
    days_late_allowed=3,
    max_session_days=3,
    app_ids=[],
    snowplow_events_database=none,
    snowplow_events_schema="atomic",
    snowplow_events_table="events",
    entities_or_sdes=none,
    custom_sql=none
) %}
    {{
        return(
            adapter.dispatch(
                "base_create_snowplow_events_this_run", "snowplow_utils"
            )(
                sessions_this_run_table,
                session_identifiers,
                session_sql,
                session_timestamp,
...
{% macro default__base_create_snowplow_events_this_run(
    ...
    {%- set lower_limit, upper_limit = snowplow_utils.return_limits_from_model(
        ref(sessions_this_run_table), "start_tstamp", "end_tstamp"
    ) %}
    {% set sessions_this_run = ref(sessions_this_run_table) %}
    {% set snowplow_events = api.Relation.create(
        database=snowplow_events_database,
        schema=snowplow_events_schema,
...
{% macro postgres__base_create_snowplow_events_this_run(
    sessions_this_run_table,
    session_identifiers,
    session_sql,
    session_timestamp,
    ...
    {% if session_identifiers %}
        {% for identifier in session_identifiers %}
            {% if identifier["schema"] | lower != "atomic" and identifier[
                "schema"
            ] not in unique_session_identifiers %}
                {% do unique_session_identifiers.update(
                    {identifier["schema"]: identifier}
                ) %}
            {%- endif -%}
            {% if identifier["schema"] in unique_session_identifiers.keys() %}
                {% if identifier["alias"] != unique_session_identifiers[
                    identifier["schema"]
                ]["alias"] or identifier[
                    "prefix"
                    ...
                    {% do exceptions.warn(
                        "Snowplow Warning: Duplicate context ( "
                        ~ identifier["schema"]
                        ~ " ) detected for session identifiers, using first alias and prefix provided ( "
                        ~ unique_session_identifiers[identifier["schema"]]
                        ~ " ) in base events this run."
                    ) %}
                {% endif %}
            {% endif %}
        {% endfor %}
    ...
    {% if entities_or_sdes %}
        ...
        {% for ent_or_sde in entities_or_sdes %}
            ...
            {% if ent_or_sde["schema"] in unique_session_identifiers.keys() %}
                {% if ent_or_sde["alias"] != unique_session_identifiers[
                    ent_or_sde["schema"]
                ]["alias"] or ent_or_sde[
                    "prefix"
                ] != unique_session_identifiers[
                    ...
                    {% do exceptions.warn(
                        "Snowplow Warning: Context or SDE ( "
                        ~ ent_or_sde["schema"]
                        ~ " ) used for session_identifier is being included, using alias and prefix from session_identifier ( "
                        ~ unique_session_identifiers[ent_or_sde["schema"]]
                        ~ " )."
                    ) %}
                {% endif %}
            {% endif %}
        ...
        {% if ent_sde_names | unique | list | length != entities_or_sdes | length %}
            {% do exceptions.raise_compiler_error(
                "There are duplicate schema names in your provided `entities_or_sdes` list. Please correct this before proceeding."
            ) %}
        {% endif %}
    {% endif %}
