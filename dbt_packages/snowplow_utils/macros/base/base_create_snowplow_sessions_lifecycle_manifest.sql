...
{% macro base_create_snowplow_sessions_lifecycle_manifest(
    ...
    event_limits_table="snowplow_base_new_event_limits",
    incremental_manifest_table="snowplow_incremental_manifest",
    package_name="snowplow"
) %}
    {{
        return(
            adapter.dispatch(
                "base_create_snowplow_sessions_lifecycle_manifest", "snowplow_utils"
            )(
                session_identifiers,
                session_sql,
                session_timestamp,
...
{% macro default__base_create_snowplow_sessions_lifecycle_manifest(
    ...
    {% set lower_limit, upper_limit, _ = snowplow_utils.return_base_new_event_limits(
        base_event_limits
    ) %}
    {% set session_lookback_limit = snowplow_utils.get_session_lookback_limit(
        lower_limit
    ) %}
    {% set is_run_with_new_events = snowplow_utils.is_run_with_new_events(
        package_name, event_limits_table, incremental_manifest_table
    ) %}
...
{% macro postgres__base_create_snowplow_sessions_lifecycle_manifest(
    session_identifiers,
    session_sql,
    session_timestamp,
    user_identifiers,
