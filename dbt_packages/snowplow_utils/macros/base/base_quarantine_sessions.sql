...
#}
{% macro base_quarantine_sessions(
    max_session_length,
    quarantined_sessions="snowplow_base_quarantined_sessions",
    src_relation=this
) %}

    {{
        return(
            adapter.dispatch("base_quarantine_sessions", "snowplow_utils")(
                max_session_length, quarantined_sessions, src_relation
            )
        )
    }}
...
{% macro default__base_quarantine_sessions(
    max_session_length, quarantined_sessions_str, src_relation
) %}
    {% set quarantined_sessions = ref(quarantined_sessions_str) %}

    {% set sessions_to_quarantine_sql = snowplow_utils.base_get_quarantine_sql(
        src_relation, max_session_length
    ) %}
...
{% macro postgres__base_quarantine_sessions(
    max_session_length, quarantined_sessions_str, src_relation
) %}
    ...
    where
        session_identifier
        in (select session_identifier from {{ sessions_to_quarantine_tmp }})
    ;
