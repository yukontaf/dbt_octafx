...
{% set dummy_graph = {
    "nodes": {
        "model.snowplow.enabled_model_w_dependency": {
            ...
            "depends_on": {
                "nodes": [
                    "model.snowplow_utils_integration_tests.snowplow_base_events_this_run"
                ]
            },
            "resource_type": "model",
        ...
        "model.snowplow.enabled_model_wo_dependency": {
            "config": {"enabled": true},
            "depends_on": {
                "nodes": ["model.snowplow_web.snowplow_web_sessions"]
            },
            "resource_type": "model",
            "tags": [var("tag_var", "snowplow_incremental")],
...
{% set actual_enabled_models = snowplow_utils.get_enabled_snowplow_models(
    "snowplow", graph_object=dummy_graph
) %}
...
{% else %}
    {# Test 1 #}
    {% set expected_enabled_models = [
        "enabled_model_w_dependency",
        "enabled_model_wo_dependency",
    ] %}
{% endif %}


{% if actual_enabled_models == expected_enabled_models %} select 1 as result limit 0  {# returns no rows therefore test passes #}
{% else %} select 1 as result  {# returns rows therefore test fails #}
{% endif %}
