...
{% for i in range(test_data.min_last_success | length) %}
    ...
    {%- else -%}
        {%- set actual_lower_limit = none -%} {%- set actual_upper_limit = none -%}
    {%- endif -%}

    {# union expected vs. actual for each test case #}
    select
        {{ snowplow_utils.cast_to_tstamp(expected_lower_limit) }}
        as expected_lower_limit,
        {{ snowplow_utils.cast_to_tstamp(expected_upper_limit) }}
        as expected_upper_limit,
