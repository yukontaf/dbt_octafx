...
{% set sessions_lifecycle_manifest_query = (
    snowplow_utils.base_create_snowplow_sessions_lifecycle_manifest(
        session_identifiers=snowplow_session_identifiers,
        session_sql=snowplow_session_sql,
        session_timestamp=var(
            "snowplow__session_timestamp", "collector_tstamp"
        ),
        user_identifiers=snowplow_user_identifiers,
        quarantined_sessions=var(
            "snowplow__quarantined_sessions",
            "snowplow_base_quarantined_sessions_actual",
        ),
        ...
        event_limits_table=var(
            "snowplow__event_limits", "snowplow_base_new_event_limits_actual"
        ),
        incremental_manifest_table=var(
            "snowplow__incremental_manifest",
            "snowplow_incremental_manifest_actual",
        ),
    )
) %}
