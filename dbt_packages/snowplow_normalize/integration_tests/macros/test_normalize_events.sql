...
{% macro test_normalize_events() %}
    ...
    {{
        return(
            adapter.dispatch(
                "test_normalize_events", "snowplow_normalize_integration_tests"
            )()
        )
    }}
...
{% macro bigquery__test_normalize_events() %}
    ...
    {% set expected_dict = {
        ...
        ~ "`."
        ~ target.dataset
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name','page_ping')",
    ...
    {% set results_dict = {
        "flat_cols_only": snowplow_normalize.normalize_events(
            ["event_name"], ["app_id"], [], [], [], [], [], [], [], [], true
        ).split()
        | join(" "),
        "sde_plus_cols": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
        ...
        | join(" "),
        "sde_plus_cols_w_alias": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
            ["UNSTRUCT_EVENT_TEST_1_0_1"],
            [["testId", "testClass"]],
        ...
        | join(" "),
        "sde_plus_1_context": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
            ["UNSTRUCT_EVENT_TEST_1_0_1"],
            [["testId", "testClass"]],
        ...
        | join(" "),
        "sde_plus_2_context": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
            ["UNSTRUCT_EVENT_TEST_1_0_1"],
            [["testId", "testClass"]],
        ...
        | join(" "),
        "sde_plus_2_context_w_alias": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
            ["UNSTRUCT_EVENT_TEST_1_0_1"],
            [["testId", "testClass"]],
        ...
        | join(" "),
        "context_only": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
            [],
            [],
        ...
        | join(" "),
        "multiple_base_events": snowplow_normalize.normalize_events(
        ...
        | join(" "),
        "multiple_sde_events": snowplow_normalize.normalize_events(
            ["event_name"],
            ["app_id"],
            ["UNSTRUCT_EVENT_TEST_1_0_1", "UNSTRUCT_EVENT_TEST2_1_0_1"],
            [["testId", "testClass"], ["testWord", "testIdea"]],
            [["number", "string"]],
            ["test1", "test2"],
            ["CONTEXTS_TEST_1_0_0", "CONTEXTS_TEST2_1_0_5"],
            [
...
{% macro databricks__test_normalize_events() %}
    ...
    {% set expected_dict = {
        "flat_cols_only": "select event_id , collector_tstamp , DATE(collector_tstamp) as collector_tstamp_date -- Flat columns from event table , app_id -- self describing events columns from event table -- context column(s) from the event table from `"
        ~ target.catalog
        ~ "`."
        ~ target.schema
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name')",
        "sde_plus_cols": "select event_id , collector_tstamp , DATE(collector_tstamp) as collector_tstamp_date -- Flat columns from event table , app_id -- self describing events columns from event table , UNSTRUCT_EVENT_TEST_1.test_id as test_id , UNSTRUCT_EVENT_TEST_1.test_class as test_class -- context column(s) from the event table from `"
        ~ target.catalog
        ~ "`."
        ~ target.schema
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name')",
        "sde_plus_cols_w_alias": "select event_id , collector_tstamp , DATE(collector_tstamp) as collector_tstamp_date -- Flat columns from event table , app_id -- self describing events columns from event table , UNSTRUCT_EVENT_TEST_1.test_id as my_alias_test_id , UNSTRUCT_EVENT_TEST_1.test_class as my_alias_test_class -- context column(s) from the event table from `"
        ~ target.catalog
        ~ "`."
        ~ target.schema
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name')",
...
{% macro snowflake__test_normalize_events() %}
    ...
    {% set expected_dict = {
        ...
        ~ target.schema
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name')",
        "sde_plus_cols": "select event_id , collector_tstamp -- Flat columns from event table , app_id -- self describing events columns from event table , UNSTRUCT_EVENT_TEST_1:testId::string as test_id , UNSTRUCT_EVENT_TEST_1:testClass::boolean as test_class -- context column(s) from the event table from "
        ~ target.database
        ~ "."
        ~ target.schema
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name')",
        "sde_plus_cols_w_alias": "select event_id , collector_tstamp -- Flat columns from event table , app_id -- self describing events columns from event table , UNSTRUCT_EVENT_TEST_1:testId::string as my_alias_test_id , UNSTRUCT_EVENT_TEST_1:testClass::boolean as my_alias_test_class -- context column(s) from the event table from "
        ~ target.database
        ~ "."
        ~ target.schema
        ~ "_scratch.snowplow_normalize_base_events_this_run where event_name in ('event_name')",
