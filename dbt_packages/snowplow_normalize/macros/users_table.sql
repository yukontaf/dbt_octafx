...
{% macro users_table(
    ...
    flat_cols=[],
    remove_new_event_check=false
) %}
    {{
        return(
            adapter.dispatch("users_table", "snowplow_normalize")(
                user_id_field,
                user_id_sde,
                user_id_context,
                user_cols,
                ...
                user_id_alias,
                flat_cols,
                remove_new_event_check,
            )
        )
    }}
{% endmacro %}

{% macro snowflake__users_table(
    user_id_field="user_id",
    user_id_sde="",
    user_id_context="",
    user_cols=[],
    ...
    {%- if user_id_sde != "" and user_id_context != "" -%}
        {% do exceptions.warn(
            "Snowplow: Both a user_id sde column and context column provided, only the sde column will be used."
        ) %}
    {%- endif -%}
    ...
    with
        defined_user_id as (
            select
                {% if user_id_sde == "" and user_id_context == "" %}
                    {{ snowplow_normalize.snakeify_case(user_id_field) }}
                    as {{ snake_user_id }}  {# Snakeify case of standard column even in snowflake #}
                {% elif user_id_sde != "" %}
                    {{ "_".join(user_id_sde.split("_")[:-2]) }}:{{ user_id_field }}
                    ::string as {{ snake_user_id }}
                {% elif user_id_context != "" %}
                    {{ "_".join(user_id_context.split("_")[:-2]) }} [
                        0
                    ]:{{ user_id_field }}::string as {{ snake_user_id }}
                {%- endif %},
                collector_tstamp as latest_collector_tstamp
                -- Flat columns from event table
                {% if flat_cols | length > 0 %}
                    {%- for col in flat_cols -%}, {{ col }} {% endfor -%}
                {%- endif -%}
                -- user column(s) from the event table
                {% if user_cols_clean | length > 0 %}
                    {%- for col, col_ind in zip(
                        user_cols_clean, range(user_cols_clean | length)
                    ) -%}  {# Loop over each context column provided #}
                        {%- for key, type in zip(
                            user_keys[col_ind], user_types[col_ind]
                        ) -%}  {# Loop over the keys in each column #}
                            ,
                            {{ col }} [0]:{{ key }}::{{ type }}
                            as {{ snowplow_normalize.snakeify_case(key) }}
                        {% endfor -%}
                    {%- endfor -%}
                {%- endif %}
    ...
    select *
    from defined_user_id
    where {{ snake_user_id }} is not null
    qualify
        row_number() over (
            partition by {{ snake_user_id }} order by latest_collector_tstamp desc
        )
        = 1
{% endmacro %}


{% macro bigquery__users_table(
    user_id_field="user_id",
    user_id_sde="",
    user_id_context="",
    user_cols=[],
    ...
    {%- for ind1 in range(user_keys | length) -%}
        ...
        {%- for ind2 in range(user_keys[ind1] | length) -%}
            {% do user_key_clean.append(
                snowplow_normalize.snakeify_case(user_keys[ind1][ind2])
            ) -%}
        {%- endfor -%}
    ...
    with
        defined_user_id as (
            select
                ...
                {% if user_cols | length > 0 %}
                    {%- for col, col_ind in zip(
                        user_cols_clean, range(user_cols | length)
                    ) -%}  {# Loop over each context column, getting the coalesced version#}
        ...
        users_ordering as (
            select
                a.*,
                row_number() over (
                    partition by {{ snake_user_id }}
                    order by latest_collector_tstamp desc
                ) as rn
            from defined_user_id a
            where {{ snake_user_id }} is not null
        )
