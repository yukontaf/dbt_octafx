{% macro compare_which_columns_differ(
    a_relation, b_relation, primary_key, exclude_columns=[]
) %}
    {{
        return(
            adapter.dispatch("compare_which_columns_differ", "audit_helper")(
                a_relation, b_relation, primary_key, exclude_columns
            )
        )
    }}
...
{% macro default__compare_which_columns_differ(
    a_relation, b_relation, primary_key, exclude_columns=[]
) %}
        ...
        bool_or as (
            ...
            select
                ...
                or (a.{{ column_name }} is not null and b.{{ column_name }} is null))
                    {% endset %},
                    {{ dbt.bool_or(compare_statement) }}
                    as {{ column | lower }}_has_difference

                {% endfor %}
            from {{ a_relation }} as a
            inner join
                {{ b_relation }} as b on a.{{ primary_key }} = b.{{ primary_key }}
