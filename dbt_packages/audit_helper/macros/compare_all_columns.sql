{% macro compare_all_columns(
    ...
    {{
        return(
            adapter.dispatch("compare_all_columns", "audit_helper")(
                a_relation, b_relation, primary_key, exclude_columns, summarize
            )
        )
    }}
{%- endmacro %}
                ...
                {%- if summarize %}
                    ...
                    final as (
                        select
                            ...
                            sum(
                                case when perfect_match then 1 else 0 end
                            ) as perfect_match,
                            sum(case when null_in_a then 1 else 0 end) as null_in_a,
                            sum(case when null_in_b then 1 else 0 end) as null_in_b,
                            sum(
                                case when missing_from_a then 1 else 0 end
                            ) as missing_from_a,
                            sum(
                                case when missing_from_b then 1 else 0 end
                            ) as missing_from_b,
                            sum(
                                case when conflicting_values then 1 else 0 end
