...
{%- macro measure_row_count(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch("measure_row_count", macro_namespace="dbt_profiler")(
...
{%- macro measure_not_null_proportion(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch(
                "measure_not_null_proportion", macro_namespace="dbt_profiler"
            )(column_name, data_type)
        )
    }}
{%- endmacro -%}
...
{%- macro measure_distinct_proportion(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch(
                "measure_distinct_proportion", macro_namespace="dbt_profiler"
            )(column_name, data_type)
        )
    }}
{%- endmacro -%}
...
{%- macro measure_distinct_count(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch(
                "measure_distinct_count", macro_namespace="dbt_profiler"
            )(column_name, data_type)
        )
    }}
{%- endmacro -%}
...
{%- macro measure_is_unique(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch("measure_is_unique", macro_namespace="dbt_profiler")(
                column_name, data_type
            )
        )
    }}
...
{%- macro sqlserver__measure_is_unique(column_name, data_type) -%}
    case
        when count(distinct {{ adapter.quote(column_name) }}) = count(*) then 1 else 0
    end
{%- endmacro -%}
...
{%- macro measure_min(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch("measure_min", macro_namespace="dbt_profiler")(
                column_name, data_type
            )
        )
    }}
...
{%- macro default__measure_min(column_name, data_type) -%}
    {%- if (
        dbt_profiler.is_numeric_dtype(data_type)
        or dbt_profiler.is_date_or_time_dtype(data_type)
    ) and not dbt_profiler.is_struct_dtype(data_type) -%}
        cast(min({{ adapter.quote(column_name) }}) as {{ dbt_profiler.type_string() }})
    {%- else -%} cast(null as {{ dbt_profiler.type_string() }})
    {%- endif -%}
{%- endmacro -%}
...
{%- macro measure_max(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch("measure_max", macro_namespace="dbt_profiler")(
                column_name, data_type
            )
        )
    }}
...
{%- macro measure_avg(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch("measure_avg", macro_namespace="dbt_profiler")(
                column_name, data_type
            )
        )
    }}
...
{%- macro default__measure_avg(column_name, data_type) -%}
    ...
    {%- if dbt_profiler.is_numeric_dtype(
        data_type
    ) and not dbt_profiler.is_struct_dtype(data_type) -%}
...
{%- macro measure_median(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch("measure_median", macro_namespace="dbt_profiler")(
                column_name, data_type
            )
        )
    }}
...
{%- macro measure_std_dev_population(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch(
                "measure_std_dev_population", macro_namespace="dbt_profiler"
            )(column_name, data_type)
        )
    }}
{%- endmacro -%}
...
{%- macro measure_std_dev_sample(column_name, data_type) -%}
    {{
        return(
            adapter.dispatch(
                "measure_std_dev_sample", macro_namespace="dbt_profiler"
            )(column_name, data_type)
        )
    }}
{%- endmacro -%}
