    ...
    {% if relation is none and relation_name is none %}
        {{
            exceptions.raise_compiler_error(
                "Either relation or relation_name must be specified."
            )
        }}
    {% endif %}

    {% if relation is none %}
        {% if schema is none %} {% set schema = target.schema %} {% endif %}

        {% if database is none %} {% set database = target.database %} {% endif %}

        {{
            log(
                "Get relation %s (database=%s, schema=%s)"
                | format(
                    adapter.quote(relation_name),
        ...
        {%- set relation = adapter.get_relation(
            database=database, schema=schema, identifier=relation_name
        ) -%}
        {% if relation is none %}
            {{
                exceptions.raise_compiler_error(
...
{%- macro assert_relation_exists(relation) -%}
    {{
        return(
            adapter.dispatch(
                "assert_relation_exists", macro_namespace="dbt_profiler"
            )(relation)
        )
    }}
{%- endmacro -%}
