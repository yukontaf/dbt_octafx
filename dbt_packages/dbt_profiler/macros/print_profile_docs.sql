{% macro print_profile_docs(
    ...
    {% if execute %}
        ...
        {%- set results = dbt_profiler.get_profile_table(
            relation=relation,
        ...
        {%- if flags.WHICH == "rpc" -%} {%- set is_dbt_cloud = true -%}
        {%- else -%} {%- set is_dbt_cloud = false -%}
        {%- endif -%}
        ...
        {% else %}
            ...
            {% for row in results.rows %}
                {%- set list_row = [""] -%}
                {% for val in row.values() %} {% do list_row.append(val) %} {% endfor %}
