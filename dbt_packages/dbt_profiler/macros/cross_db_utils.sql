...
{%- macro default__type_string() -%} varchar {%- endmacro -%}

{%- macro bigquery__type_string() -%} string {%- endmacro -%}
...
{%- macro is_numeric_dtype(dtype) -%}
    {{
        return(
            adapter.dispatch("is_numeric_dtype", macro_namespace="dbt_profiler")(
                dtype
            )
        )
    }}
...
{%- macro is_logical_dtype(dtype) -%}
    {{
        return(
            adapter.dispatch("is_logical_dtype", macro_namespace="dbt_profiler")(
                dtype
            )
        )
    }}
...
{%- macro default__is_logical_dtype(dtype) -%}
    {% set is_bool = dtype.startswith("bool") %} {% do return(is_bool) %}
{%- endmacro -%}
...
{%- macro is_date_or_time_dtype(dtype) -%}
    {{
        return(
            adapter.dispatch("is_date_or_time_dtype", macro_namespace="dbt_profiler")(
                dtype
            )
        )
    }}
...
{%- macro default__is_date_or_time_dtype(dtype) -%}
    {% set is_date_or_time = dtype.startswith("timestamp") or dtype.startswith(
        "date"
    ) %}
...
{%- macro is_struct_dtype(dtype) -%}
    {{
        return(
            adapter.dispatch("is_struct_dtype", macro_namespace="dbt_profiler")(dtype)
        )
    }}
{%- endmacro -%}
...
{%- macro information_schema(relation) -%}
    {{
        return(
            adapter.dispatch("information_schema", macro_namespace="dbt_profiler")(
                relation
            )
        )
    }}
...
{%- macro default__select_from_information_schema_columns(relation) -%}
    ...
    where
        lower(table_schema) = lower('{{ relation.schema }}')
