{{
    config(
        ...
        partition_by=(
            {"field": "event_day", "data_type": "date"}
...
with
    ...
    {% if is_incremental() %}
        ...
        max_date as (select max(event_day) as max_event_day from {{ this }}),
    ...
    event_data_raw as (
        ...
        {% if is_incremental() %}
                , max_date where event_day >= max_date.max_event_day
    ...
    event_type as (select * from {{ var("event_type") }}),

    session_data as (select * from {{ ref("amplitude__sessions") }}),
    ...
    event_enhanced as (
        ...
        select
            ...
            cast(
                event_data.amplitude_user_id as {{ dbt.type_string() }}
            ...
            {% if var("event_properties_to_pivot") %}
                ...
                {{
                    fivetran_utils.pivot_json_extract(
                        ...
                        list_of_properties=var("event_properties_to_pivot"),
                    )
                }}
            {% endif %},
            ...
            row_number() over (
                partition by session_id order by event_time asc
            ) as session_event_number,
            row_number() over (
                partition by amplitude_user_id order by event_time asc
            ...
            {% if var("group_properties_to_pivot") %}
                ...
                {{
                    fivetran_utils.pivot_json_extract(
                        ...
                        list_of_properties=var("group_properties_to_pivot"),
                    )
                }}
            {% endif %},
            ...
            {% if var("user_properties_to_pivot") %}
                ...
                {{
                    fivetran_utils.pivot_json_extract(
                        ...
                        list_of_properties=var("user_properties_to_pivot"),
                    )
                }}
            {% endif %},
        ...
        from event_data
        left join event_type on event_data.event_type_id = event_type.event_type_id
        left join
            session_data
    ...
    final as (
        ...
        select
            *,
            {{ dbt_utils.generate_surrogate_key(["unique_event_id", "event_day"]) }}
            as unique_key
        from event_enhanced
    )
