...
with
    event_enhanced as (select * from {{ ref("amplitude__event_enhanced") }}),
            ...
            count(
                ...
                (
                    case
                        when
                            cast(
                                {{ dbt.date_trunc("day", "user_creation_time") }}
                                as date
                            )
                            = event_day
    ...
    final as (
        ...
        select
            ...
            {{ dbt_utils.generate_surrogate_key(["event_day", "event_type"]) }}
            as daily_unique_key
        from spine_joined
        ...
        {% if is_incremental() %}
            ...
            where
                event_day
                >= coalesce((select max(event_day) from {{ this }}), '2020-01-01')
