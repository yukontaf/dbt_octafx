{% macro funnel(steps=none, event_stream=none, start_date=none, end_date=none) %}
    {{
        return(
            adapter.dispatch("funnel", "dbt_product_analytics")(
...
{% macro default__funnel(steps, event_stream, start_date, end_date) %}
    with
        event_stream
        as {{
            dbt_product_analytics._select_event_stream(
                event_stream, start_date, end_date
            )
        }}
        {% for step in steps %}
            ,
            event_stream_step_{{ loop.index }} as (
                ...
                {% if loop.index > 1 %}
                    inner join
                        event_stream_step_{{ loop.index - 1 }} as previous_events
        ...
        final as (
            select
                ...
                * unique_users
                / nullif(
                    first_value(unique_users) over (order by step_index), 0
                ) as pct_conversion,
                1.0
                * unique_users
                / nullif(
                    lag(unique_users) over (order by step_index), 0
                ) as pct_of_previous
            from event_funnel
        )
...
{% macro snowflake__funnel(steps, event_stream, start_date, end_date) %}
    with
        ...
        steps as (
            {% for step in steps %}
                select '{{ step }}' as event_type, {{ loop.index }} as index
                {% if not loop.last %}
                    union all
                {% endif %}
            {% endfor %}
                ...
                match_recognize(
                    ...
                    order by
                        ...
                        pattern(
                            {% for step in steps %}
                                (
                                    {% for i in range(loop.length - loop.index + 1) %}
                                        step_{{ loop.index }}+
                                    {% endfor %}
                                )
                        ...
                        define
                        {% for step in steps %}
                            step_{{ loop.index }} as event_type = '{{ step }}'
                            {% if not loop.last %}, {% endif %}
                        {% endfor %}
                )
            group by event_type
        ...
        final as (
            select
                event_funnel.event_type,
                unique_users,
                cast(unique_users as double) / nullif(
                    first_value(unique_users) over (order by steps.index), 0
                ) as pct_conversion,
                1.0
                * cast(unique_users as double)
                / nullif(
                    lag(unique_users) over (order by steps.index), 0
                ) as pct_of_previous
            from event_funnel
            left join steps on event_funnel.event_type = steps.event_type
