{% macro retention(
    ...
    {% if event_stream is none %}
        {{
            exceptions.raise_compiler_error(
                "parameter 'event_stream' must be provided"
            )
        }}
    {% endif %}
    ...
    {% if first_action is none %}
        {{
            exceptions.raise_compiler_error(
                "parameter 'first_action' must be provided"
            )
        }}
    {% endif %}
    ...
    {% if second_action is none %}
        {{
            exceptions.raise_compiler_error(
                "parameter 'second_action' must be provided"
            )
        }}
    {% endif %}
    ...
    with
        ...
        first_events as (
            select
                {% if group_by %} dimension, {% endif %} count(*) as unique_users_total
        ...
        {% for period in periods %}
            ...
            secondary_events_{{ period }} as (
                ...
                inner join
                    ...
                    {% if group_by %}
                        and event_stream.{{ group_by }} = first_event_users.dimension
                    {% endif %}
                where
                    event_type = '{{ second_action }}'
                    and event_date
                    >= {{
                        dbt_product_analytics._dateadd(
                            datepart=period_type,
                            interval=period,
                            from_date_or_timestamp="first_event_date",
                        )
                    }}
