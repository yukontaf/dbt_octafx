{% macro timestamp_diff(first_date, second_date, datepart) %}
    {{
        adapter.dispatch("timestamp_diff", "fivetran_utils")(
...
{% macro bigquery__timestamp_diff(first_date, second_date, datepart) %}
    ...
    timestamp_diff({{ second_date }}, {{ first_date }}, {{ datepart }})
...
{% macro postgres__timestamp_diff(first_date, second_date, datepart) %}
    ...
    {% elif datepart == "quarter" %}
        (
            {{ dbt.datediff(first_date, second_date, "year") }} * 4
            + date_part('quarter', ({{ second_date }})::date)
            - date_part('quarter', ({{ first_date }})::date)
        )
    {% elif datepart == "month" %}
        (
            {{ dbt.datediff(first_date, second_date, "year") }} * 12
