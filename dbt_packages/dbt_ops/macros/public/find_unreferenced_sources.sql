...
#}
{% macro find_unreferenced_sources(
    format="text",
    reference_resource_types=["model", "snapshot", "exposure"]
) %}
    {#
  ...
  #}
    {% set unreferenced_sources = __find_unreferenced_sources(
        reference_resource_types
    ) %}

    {% if format == "text" %}
        {% for unique_id in unreferenced_sources %} {{ print(unique_id) }} {% endfor %}
    {% elif format == "json" %} {{ print(tojson(unreferenced_sources)) }}
    {% elif format == "yaml" %} {{ print(toyaml(unreferenced_sources)) }}
    {% else %} {{ exceptions.raise_compiler_error("Invalid `format`. Got: " ~ format) }}
    {% endif %}
{% endmacro %}

{% macro __find_unreferenced_sources(
    reference_resource_types=["model", "snapshot", "exposure"]
) %}
    {# Get all sources #}
    {% set all_sources = {} %}
