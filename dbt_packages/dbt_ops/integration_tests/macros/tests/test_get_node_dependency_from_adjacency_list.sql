...
{% macro test_get_node_dependency_from_adjacency_list() %}
    ...
    {% set depends_on = node_dependency_info[
        "model.integration_tests.stg_customers"
    ]["depends_on"].keys() %}
    {% if depends_on | length != 1 %}
        {{
            exceptions.raise_compiler_error(
                "The length of depends_on is not 1. But, " + depends_on | length
            )
        }}
    {% endif %}
    {% if "source.integration_tests.main.raw_customers" not in depends_on %}
        {{
            exceptions.raise_compiler_error(
                "The key 'source.integration_tests.main.raw_customers' is not found in depends_on."
            )
        }}
    {% endif %}
{% endmacro %}
