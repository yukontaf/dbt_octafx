...
#}
    {% if var("segment_page_views_table") is string %}
        ...
        unioned_sources as (
            select cast('segment_page_views_table' as text) as source_name, *
    ...
    row_numbering as (
        ...
        select
            *,
            row_number() over (
                partition by source_name, id order by received_at asc
            ) as row_num
        from unioned_sources

    ),

    deduped as (select * from row_numbering where row_num = 1),
