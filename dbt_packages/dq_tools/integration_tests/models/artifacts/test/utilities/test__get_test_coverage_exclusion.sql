select
    'integration test - default' as test_case,
    ...
    '{{ ("{' databases ': [' dummy '], ' schemas ': [' " | escape) 
            ...
            ~ (" '], ' tables ': [' dq_issue_log ', ' dummy ']}" | escape) }}'
    as expected

    {%- set test_input -%} {%- endset %}
    {%- set rule = (fromyaml(test_input) or {}).get(
...
union all
select
    'multiple values accros all arguments' as test_case,
    '{{ rule | escape }}' as test_input,
    '{{ dq_tools.__get_test_coverage_exclusion(rule=rule) | escape }}' as actual,
