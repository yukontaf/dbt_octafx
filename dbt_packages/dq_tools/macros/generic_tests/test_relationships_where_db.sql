{% test relationships_where_db(model, to, field) %}
    ...
    with
        left_table as (
            ...
            select {{ column_name }} as ref_id

            from {{ model }}

            where
                {{ column_name }} is not null and {{ from_condition }}
                {% if where %} and {{ where }} {% endif %}
    ...
    select *
    from left_table
    left join right_table on left_table.ref_id = right_table.ref_id
