{% test equal_rowcount_where_db(model, compare_model) %}
    ...
    {% set compare_columns_csv = kwargs.get(
        "compare_columns_csv", kwargs.get("columns_csv", "*")
    ) %}
    ...
    with
        left_table as (
            ...
            select count({{ columns_csv }}) as count_a
        ...
        right_table as (
            ...
            select count({{ compare_columns_csv }}) as count_b
