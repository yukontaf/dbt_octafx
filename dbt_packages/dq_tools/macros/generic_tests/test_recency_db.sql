{% test recency_db(model, datepart, interval) %}
   ...
   {{ dq_tools.transaction_begin() }}
    ...
    {% endif %}
    {%- set threshold = dq_tools.dateadd(
        datepart, interval * -1, dq_tools.current_timestamp()
    ) -%}
    with max_date as (select max({{ column_name }}) max_date_value from {{ model }})
    select max_date_value,{{ threshold }} as threshold
    from max_date
