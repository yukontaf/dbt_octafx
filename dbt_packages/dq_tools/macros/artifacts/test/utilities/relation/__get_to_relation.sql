{% macro __get_to_relation(test_node) %}
    ...
    {% set to_model = (
        (
            (
                test_node.test_metadata.kwargs.to
                if test_node.test_metadata.kwargs.to is defined
                else none
            )
            or (
                test_node.test_metadata.kwargs.compare_model
                if test_node.test_metadata.kwargs.compare_model is defined
                else none
            )
        )
