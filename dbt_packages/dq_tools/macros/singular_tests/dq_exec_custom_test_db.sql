{% macro dq_exec_custom_test_db(
    business_test_code, business_test_comment, sql_cmd, severity_level
) %}
    ...
    {{ config(severity=severity_level) }}
...
commit;
    ...
    {% if execute %}
        ...
        select 1
        from
            {{ var("dbt_dq_tool_database", target.database) }}.dbt.dq_business_check_log
        where invocation_id = '{{ p_invocation_id }}' and dq_model = '{{ this }}'
    {% endif %}
