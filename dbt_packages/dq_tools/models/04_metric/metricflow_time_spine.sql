{{ config(materialized="table", tags=["semantic", "metricflow"]) }}

-- Check https://docs.getdbt.com/docs/build/metricflow-time-spine
{% set start_date = var(
    "dbt_dq_tool_start_date", "to_date('01/01/2000','mm/dd/yyyy')"
) -%}
...
with
    days as ({{ dbt_utils.date_spine("day", start_date, end_date) }}),
